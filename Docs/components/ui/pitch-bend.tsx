...
export interface PitchBendProps extends Omit<React.HTMLAttributes<HTMLDivElement>, 'onChange'> {
  spring?: boolean
...
const PitchBend = React.forwardRef<HTMLDivElement, PitchBendProps>(
  (
    {
      spring = true,
      onChange,
    ...
    const handleMove = (clientY: number) => {
      ...
      // Calculate normalized value between min and max with 0 at center
      // For a pitch bend wheel: top = positive max, center = 0, bottom = negative min
      let normalizedValue
      if (newPosition < 50) {
        // Upper half (positive values)
        normalizedValue = max * (1 - newPosition / 50)
      } else {
        // Lower half (negative values)
        normalizedValue = min * ((newPosition - 50) / 50)
      }

      // Update tilt effect based on position
      ...
      if (spring) {
        const animateSpring = () => {
          setPosition((prev) => {
            ...
            // Calculate normalized value with 0 at center
            let normalizedValue
            if (newPosition < 50) {
              // Upper half (positive values)
              normalizedValue = max * (1 - newPosition / 50)
            ...
            if (Math.abs(diff) < 0.1) {
              ...
              setValue(0) // Center is always 0 for pitch bend
              setTilt(0)
    ...
    const handleKeyDown = (e: React.KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case "ArrowDown":
          ...
          break
        case " ": // Space key
          newPosition = restPositionPercent
          break
        default:
          return
      }
